...
ENV WAREHOUSE_PATH=/home/guilherme/warehouse

# A imagem base já define o usuário 'jovyan', mas vamos criar um usuário customizado para a nossa imagem.
USER root

# Cria o usuário 'guilherme', seu diretório home e o adiciona ao grupo 'users' (GID 100)
# A UID 1001 é usada para evitar conflito com a UID do jovyan (1000)
RUN useradd -ms /bin/bash -g 100 -u 1001 guilherme && \
    mkdir -p /home/guilherme && \
    mkdir -p /home/guilherme/warehouse && \
    chown -R guilherme:users /home/guilherme && \
    chown -R guilherme:users /opt/warehouse

# Retorna para o usuário 'guilherme' e define o diretório de trabalho
USER guilherme
WORKDIR /home/guilherme/work
